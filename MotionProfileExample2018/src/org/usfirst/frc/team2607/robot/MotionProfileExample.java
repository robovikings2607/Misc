...
public class MotionProfileExample {
	...
	/**
	 * Keep track of how many trajectory points have been pushed into the Talon.
	 * The top buffer size is 512 trajectory points, so we need to have a method
	 * to handle trajectories longer than 512 trajectory points.
	 */
	private int _pointsInBuffer = 0;
	...
	public void control() {
		...
		if (_talon.getControlMode() != ControlMode.MotionProfile) {
			...
			switch (_state) {
				case 0: /* wait for application to tell us to start an MP */
					if (_bStart) {
						...
						_pointsInBuffer = 0;
						/*
						 * Clear any trajectory points in the buffer and start a new trajectory
						 */
						_talon.clearMotionProfileTrajectories();
				...
				case 2: /* check the status of the MP */
					 /*
					 * Continue filling the buffer if there are more trajectory points to push
					 */
					fillBuffer();
			...
			_encPos = _talon.getSelectedSensorPosition(0);
			_encVel = _talon.getSelectedSensorVelocity(0);

			/* printfs and/or logging */
			Instrumentation.process(_status, _pos, _vel, _heading, _encPos, _encVel);
		}
	}
	/**
	...
	private void fillBuffer() {
		...
		fillBuffer(GeneratedMotionProfile.Points, GeneratedMotionProfile.kNumPoints);
	}

	private void fillBuffer(double[][] profile, int totalCnt) {
		 ...
		 */
//		_talon.clearMotionProfileTrajectories();
//       Moved this statement to state 1 because this routing will be called several times if
//       the trajectory list is over 512 trajectory points
		...
		while ((_pointsInBuffer < totalCnt) && (_status.topBufferCnt < Constants.kTopBufferSize)) {
//			System.out.println("State " + _state + " Points in Buffer " + _pointsInBuffer + " Total Count " + totalCnt + " Buffer Count " + _status.topBufferCnt + " Buffer Size " + Constants.kTopBufferSize);
			double positionTicks = profile[_pointsInBuffer][0];
			double velocityTicksPer100ms = profile[_pointsInBuffer][1];
...
//			point.velocity = velocityRPM * Constants.kSensorUnitsPerRotation / 600.0; //Convert RPM to Units/100ms
			point.position = positionTicks;
			point.velocity = velocityTicksPer100ms;
			point.headingDeg = 0; /* future feature - not used in this example*/
			point.profileSlotSelect0 = 0; /* which set of gains would you like to use [0,3]? */
			point.profileSlotSelect1 = 0; /* future feature  - not used in this example - cascaded PID [0,1], leave zero */
			point.timeDur = GetTrajectoryDuration((int)profile[_pointsInBuffer][2]);
			point.zeroPos = false;
			if (_pointsInBuffer == 0)
				point.zeroPos = true; /* set this to true on the first point */

			point.isLastPoint = false;
			if ((_pointsInBuffer + 1) == totalCnt)
				point.isLastPoint = true; /* set this to true on the last point  */

			_talon.pushMotionProfileTrajectory(point);
			
			/*
			 * Increase the buffer pointer for the next call.
			 */
			_pointsInBuffer++;
			_talon.getMotionProfileStatus(_status);
			
			if ((_state == 0) && (_pointsInBuffer > Constants.kTopBufferSize)) {
