 ...
 */
	 ...
	 * positive voltage to the motor, it expects the encoder tick count to increase.  When the TalonSRX
	 ...
	 * you will never reach the desired poistion of +100.
 ...
 */
	...
	/*
	 * The code below instructs the TalonSRX to move to a target position depending on which
	 * button on the gamepad (joystick) is being pressed.  You have to keep the button pressed to
	 * move the wheel to the position defined for the button.  If no button is being pressed, the
	 * TalonSRX will move the wheel to the zero position.
	 */
		...
		/*
		 * If not button is being pressed, the TalonSRX returns the wheel to the initial zero
		 * position.
		 */
		 ...
		 */
		/*
		 * Here's how we set the target position to +250 ticks and the TalonSRX will move
		 * the mechanism (provide positive voltage to the motor until the encoder count 
		 * reaches the targetPosition, in this case +250)
		 */
 ...
 */
		...
		else
		{
			...
			SmartDashboard.putNumber("Button Pressed", button);
